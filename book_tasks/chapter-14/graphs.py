...
class Edge(object):
    def __init__(self, src, dest):
        """Assumes src and dest and nodes"""
        self._src = src
        self._dest = dest
    def get_source(self):
        return self._src
...
class Weighted_edge(Edge):
    def __init__(self, src, dest, weight=1.0):
        """Assumes src and dest and nodes, weight a number"""
        self._src = src
        self._dest = dest
        self._weight = weight
    def get_weight(self):
        return self._weight
    def __str__(self):
        return f"{self_.src.get_name()} ->({self._weight}) {self._dest.get_name()}"
